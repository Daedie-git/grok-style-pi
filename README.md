...
| Module | Owns |
|---|---|
| `src/extension/file-navigation.ts` | `/open`, the shortcut, edit history, Markdown file links, and the lifetime of the shared Cursor opener |
| `src/extension/session-chrome.ts` | Footer usage polling, composer installation, and shared terminal color setup and cleanup |
| `src/extension/types.ts` | The factory's dependency and session contracts |
| `src/tools.ts`, `src/diamond.ts`, `src/diff-render.ts`, `src/highlight.ts` | Tool rendering, summaries, diffs, and syntax colors |
| `src/cursor-workspace.ts`, `src/open-in-cursor.ts`, `src/compile-commands.ts` | Workspace preparation and Cursor launching |
| `src/activity.ts`, `src/activity-ui.ts`, `src/subagent-adapter.ts` | Activity presentation and in-process Pi Subagents observation |
| `src/herdr-subagents.ts`, `src/herdr-subagent-*.ts`, `src/herdr-client.ts` | The separate Herdr runner, child processes, state, persistence, and pane operations |
